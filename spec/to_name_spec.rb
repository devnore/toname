...
describe ToName do
  ...
  context 'tv show episodes' do
    ...
    it 'should handle x of y format' do
      check_to_name('/test/A History of Britain - 02 of 15 - Beginnings.avi', 'A History of Britain   02 of 15   Beginnings', 'A History of Britain', nil, 1, 2)
      check_to_name('/test/A History of Britain - 02of15 - Beginnings.avi', 'A History of Britain   02of15   Beginnings', 'A History of Britain', nil, 1, 2)
    end
  ...
  it 'should handle movie names containing just numbers' do
    check_to_name('/test/2012.(cam).avi', '2012 (cam)', '2012')
    check_to_name('/test/2012.(CAM).avi', '2012 (CAM)', '2012')
  end
  ...
  it 'should find parent folder of a media file' do
   expect(ToName.parent_folder_name('test.mpg')).to eq(nil)
   expect(ToName.parent_folder_name('/test.mpg')).to eq(nil)
   expect(ToName.parent_folder_name('/tmp/test.mpg')).to eq('tmp')
   expect(ToName.parent_folder_name('/asdf/asdf/a/tmp/test.mpg')).to eq('tmp')
   expect(ToName.parent_folder_name('tmp/test.mpg')).to eq('tmp')
   expect(ToName.parent_folder_name('/CD1/test.mpg')).to eq('CD1')
   expect(ToName.parent_folder_name('/tmp/CD1/test.mpg')).to eq('tmp')
   expect(ToName.parent_folder_name('/tmp/cd2/test.mpg')).to eq('tmp')
 end
 ...
 def check_to_name(location, raw_name, name, year=nil, series=nil, episode=nil)
  ...
  expect(c.raw_name).to eq(raw_name)
  expect(c.name).to eq(name)
  expect(c.year).to eq(year)
  expect(c.series).to eq(series)
  expect(c.episode).to eq(episode)
end
end
